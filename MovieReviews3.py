...
def ReviewMovie(FName, make_new = (False, ""), looks = False):
    ...
    if looks:
        keep_list = [i for i in range(len(WordsR)) if TweetCounter[WordsR[i]] == 1 and "looks" in WordsR[i] or "sounds"in WordsR[i] and not [j for j in avoid_words if j in Words[i]]]
    else:
        keep_list = [i for i in range(len(WordsR)) if TweetCounter[WordsR[i]] == 1 and "looks" not in WordsR[i] and "sounds" not in WordsR[i] and not [j for j in avoid_words if j in Words[i]]]
